...
export async function loadNamespace(locale: SupportedLocale, namespace: string): Promise<void> {
  ...
  try {
    ...
    if (locale !== 'en') await loadNamespace('en', namespace)
  }
}

async function importNamespace(locale: SupportedLocale, namespace: string): Promise<Record<string, unknown>> {
  const map: Record<string, Record<string, () => Promise<unknown>>> = {
    en: {
  ...
  const module = await loader() as { default?: Record<string, unknown> } & Record<string, unknown>
